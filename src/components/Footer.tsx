    ...
    <>
      <footer className="bg-[#1f1509] text-[#a08060]">
        ...
        <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <span className="text-[13px] text-[#a08060]">
            ...
            <button onClick={() => setOpen('accuracy')} className="text-[#c8a878] underline underline-offset-2 hover:text-white transition-colors">
              Learn more
            </button>
          </span>
        </div>
      ...
      {open === 'privacy' && (
        <Modal title="Privacy Policy" onClose={() => setOpen(null)}>
          ...
          <p>Voya does not collect, store, or sell any personal data. We do not require account creation and do not use tracking cookies.</p>
          <p><strong>Search queries</strong> — Country names you type are sent to our AI provider to generate travel content. They are not linked to your identity and are not retained after the response is delivered.</p>
          <p><strong>Analytics</strong> — We may use privacy-respecting, anonymous analytics (page views only) to improve the product. No personal identifiers are collected.</p>
      ...
      {open === 'terms' && (
        <Modal title="Terms of Use" onClose={() => setOpen(null)}>
          ...
          <p><strong>Changes</strong> — We reserve the right to modify these terms at any time. Continued use of the service constitutes acceptance of the updated terms.</p>
      ...
      {open === 'accuracy' && (
        <Modal title="AI Accuracy Notice" onClose={() => setOpen(null)}>
          ...
          <p>We are continuously working to improve the quality and accuracy of our outputs. If you notice an error, please let us know at <a href="mailto:Prempaudel5b@gmail.com" className="text-[#b07a3a] underline">Prempaudel5b@gmail.com</a>.</p>
        </Modal>
      )}

      {open === 'contact' && (
        <Modal title="Help & Contact" onClose={() => setOpen(null)}>
          <p>We're a small team building Voya to make travel more accessible and informed. We'd love to hear from you.</p>
          ...
          <ul className="list-disc pl-5 space-y-2">
            ...
            <li><strong>I found incorrect information.</strong> — Please report it via email so we can investigate and improve.</li>
          </ul>
          <p className="text-[#6b5740]">We aim to respond within 48 hours.</p>
        </Modal>
      )}
    </>
  );
